...
### 관리자 대시보드 (심볼 추가/삭제·현황 보기) — 내 PC에서

수집 현황을 보고 **심볼을 추가/삭제**하는 화면입니다. 내 PC에서만 돌리며 추가 설치가
필요 없습니다(파이썬만 있으면 됨).

**여는 법**: 프로젝트 폴더의 **`dashboard.bat` 더블클릭** → 브라우저가 자동으로 열립니다.
(안 열리면 주소창에 `http://127.0.0.1:8787` 입력. 끄려면 까만 창에서 `Ctrl+C`.)

**할 수 있는 것**
- **심볼 추가**: 칸에 `SOLUSDT` 처럼 입력 → `+ 추가`. 실제 존재하는 심볼인지 자동 검증합니다.
- **심볼 삭제**: 심볼 옆 `삭제` (수집만 중단, 기존 데이터 파일은 남습니다).
- **☁ 변경사항 GitHub 반영**: 추가/삭제를 실제 자동 수집(Actions)에 적용하려면 이 버튼을
  누르세요. 다음 자동 수집부터 반영됩니다. **(이 버튼을 안 누르면 내 PC 설정만 바뀝니다.)**
- **▶ 지금 수집(로컬)**: 추가한 심볼 데이터를 기다리지 않고 지금 이 PC로 받아봅니다.
- 심볼별 봉 수·기간·결측·마지막 수집, 최근 지표 차트, 수집 로그 확인.

> 💡 심볼을 새로 추가하면: ① 대시보드에서 추가 → ② `GitHub 반영` 클릭 → 다음날 자동
> 수집이 그 심볼의 과거치까지 채웁니다. 바로 보고 싶으면 `지금 수집`도 눌러 보세요.
...
| `dashboard.py` / `dashboard.html` / `dashboard.bat` | 로컬 관리자 대시보드(현황·심볼 추가/삭제·차트) |
